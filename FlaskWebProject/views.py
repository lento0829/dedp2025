...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            app.logger.warning('Invalid login attempt for username: {}'.format(form.username.data))
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        login_user(user)
        app.logger.info('User {} logged in successfully via Microsoft Authentication'.format(user.username))
        _save_cache(cache)
    return redirect(url_for('home'))
